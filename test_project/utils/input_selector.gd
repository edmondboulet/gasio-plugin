...
## Audio INPUT selector. Lists native (WASAPI/ALSA/CoreAudio) inputs AND ASIO
## drivers. The actual routing is handled by the AudioRouter autoload, which
## coordinates with the output selector — if both pick ASIO, it opens a single
## full-duplex device automatically.
...
func _populate() -> void:
	...
	var probe = ClassDB.instantiate("AsioAudioInput")
	add_child(probe)
...
func _on_item_selected(index: int) -> void:
	...
	AudioRouter.request_input(kind, meta["name"])
